...
// FIXED: Use createCipheriv instead of deprecated createCipher
export function encryptToken(token: string): string {
  const iv = crypto.randomBytes(16)
  const key = Buffer.from(ENCRYPTION_KEY).subarray(0, 32) // Ensure 32 bytes
...
// FIXED: Use createDecipheriv instead of deprecated createDecipher
